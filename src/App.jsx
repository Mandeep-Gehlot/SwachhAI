...
const App = () => {
  ...
  const analyzeWaste = async (base64, languageCode) => {
    if (!base64)
      return setError("Image data missing. Please capture or upload an image.");
    setIsLoading(true);
    setError(null);
    setPrediction(null);
    ...
    try {
      ...
      const estimatedValueINR =
        Math.round((result.estimatedValueUSD || 0) * INR_PER_USD * 100) / 100;
  ...
  const renderPredictionBox = () => {
    ...
    const isRecyclable = prediction.category
      ?.toLowerCase()
      .match(/plastic|paper|metal|glass|cardboard/);
    ...
    return (
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 w-32 h-32 blur-3xl rounded-full opacity-20 -mr-10 -mt-10 ${
            isRecyclable ? "bg-green-500" : "bg-yellow-500"
          }`}
        ></div>
      ...
      >
        ...
        <div className="mt-8 mb-6">
          ...
          <p className="text-slate-500 text-sm mt-2 font-medium">
            Identify waste, estimate value, and recycle smarter.
          </p>
        </div>
